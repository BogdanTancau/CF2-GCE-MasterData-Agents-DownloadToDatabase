...
def run_agent(request):
    ...
    try:
        ...
        for company in companies:
            ...
            companyShortPath = 'snowCacheFile/{0}'.format(company.Ident)
            ...
            for blob in blobs:
                ...
                tableName = blobName.split("_")[2]
                tableNameCache = "SNOW{0}Cache".format(tableName.capitalize())
                logging.info(f"Destination table name: {0}", tableNameCache)
                print("Destination table name: {0}".format(tableNameCache))
                ...
                df.to_sql(name=tableNameCache, con=session.get_bind(), if_exists = 'append', index=False)
                logging.info(f"File {0} uploaded to {1} table", blob.name, tableNameCache)
                print("File {0} uploaded to {1} table".format(blob.name, tableNameCache))
